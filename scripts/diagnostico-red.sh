...
log "================================================"
log "  REPORTE DE DIAGNOSTICO DE RED - $FECHA"
log "================================================"
log ""
...
for host in $HOSTS_TEST; do
    ...
    if ping -c 2 -W 3 "$host" &>/dev/null; then
        ...
        latencia=$(ping -c 2 -W 3 "$host" \
            | tail -1 \
            | awk -F '/' '{print $5}' 2>/dev/null || echo "?")
